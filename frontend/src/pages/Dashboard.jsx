        ...
        {perUnit.length === 0 ? (
          ...
          <div className="bar-chart">
            {perUnit.map((row, i) => {
              ...
              const total = formatRupiah(row.total_anggaran);
              return (
                <div className="bar-row" key={row.unit_kerja_id} tabIndex={0} aria-describedby={`bar-tip-unit-${i}`}>
                  ...
                  <div className="bar-tip" id={`bar-tip-unit-${i}`} role="tooltip">
                    <strong>{total}</strong> Anggaran
                  </div>
                </div>
              );
            })}
        ...
        {perJenis.length === 0 ? (
          ...
          <div className="bar-chart">
            {perJenis.map((row, i) => {
              ...
              const total = formatRupiah(row.total_anggaran);
              return (
                <div className="bar-row" key={row.jenis_kegiatan_id} tabIndex={0} aria-describedby={`bar-tip-jenis-${i}`}>
                  ...
                  <div className="bar-value">{total}</div>
                  <div className="bar-tip" id={`bar-tip-jenis-${i}`} role="tooltip">
                    <strong>{total}</strong> Anggaran
                  </div>
                </div>
              );
